...
class Stocks(object):
    ...
    def __init__(self, year):
        ...
        os.makedirs(self.STOCKS_BASE_PATH, exist_ok=True)
    ...
    def save_trading_data(self, ticker_sym):
        ...
        try:
            ...
            self.get_current_stock().to_csv(self.STOCKS_BASE_PATH + f"/{ticker_sym}.csv", index=True)
    ...
    def get_trading_history(self, ticker, startDt, endDt):
        ...
        # open the source file and return it as a DF
        try:
            trading_history = pd.read_csv(f"datasets/stocks/{ticker}.csv", index_col=[0], parse_dates=True)
